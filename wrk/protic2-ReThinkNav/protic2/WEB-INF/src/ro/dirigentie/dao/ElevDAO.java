 ...
 */
  public class ElevDAO extends GenericDAO{
    private static List elements = null;

   	private static final String INSERT_SQL = "INSERT INTO elev (id, matr, nume, datan, sex, adr, loc, jud, codp, tel, email, statut, nat ) " +
			"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	private static final String UPDATE_SQL = "UPDATE elev SET  matr=?, nume=?, datan=?, sex=?, adr=?, loc=?, jud=?, codp=?, tel=?, email=?, statut=?, nat=?  WHERE " +
			" id=? ";

	private static final String SELECT_SQL = "SELECT * FROM elev";

	private static final String SELECT_BY_PK_SQL = "SELECT * FROM elev WHERE " +
			" id=? ";

    private static final String DELETE = "DELETE FROM elev WHERE " +
            " id=? ";
    int[] a = new int[23];
   ...
   /* private static final String SELECT_LOGIN_SQL = "SELECT * FROM elev where username=? and userpassword=? ";  */
    ...
    private static final String COUNT_SQL = "SELECT count(*) FROM elev";


    private static final String DELETE_BY_PK_SQL = "DELETE FROM elev WHERE " +
            " id=? ";
