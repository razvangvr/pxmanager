...
<h2 align=center>Asociaza  elevi grupa nr. <%=request.getParameter("id_grupa") %>, profesor <%=((ProfesorBean)new ProfesorDAO().getByPK(dpb.getId_profesor()+"").get(0)).getNume()%> disciplina <%=((DisciplinaBean)dDao.getByPK(dpb.getId_disciplina()+"").get(0)).getDen()%></h2>
...
<table width="400" border="0" cellpadding="0">
  ...
  <tr>
    ...
    <input type="button" name="cancel" value="Inapoi" onClick="document.location='list.jsp?id=<%=request.getParameter("id")%>&id_grupa=<%=request.getParameter("id_grupa")%>'">
...
<%
            ...
            EleviclaseDAO eleviclasaDAO = new EleviclaseDAO();
            ElevigrupeDAO elevigrupeDAO = new ElevigrupeDAO();
            List elements = eleviclasaDAO.getByClasa(dpb.getId_clasa()+"");
            System.out.println("elements list clasa size " + elements.size());
            if (elements!=null)
            for (int i = 0; i < elements.size(); i++) {
                EleviclasaBean sb = (EleviclasaBean) elements.get(i);
                List selected = elevigrupeDAO.getByIdDscprofAndElev(dpb.getId()+"", (int)sb.getId_elev());
                if(selected == null || selected.size()  == 0) {   %>
                    <option name="DA" type="text"  value="<%=sb.getId_elev()%>"><%=((ElevBean)eDao.getByPK(sb.getId_elev()+"").get(0)).getNume()%></option>
<%              }
            }
%>
        </select>
