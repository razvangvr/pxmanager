...
import ro.auto.beans.DisciplinaBean;
 ...
 */
  public class DisciplinaDAO extends GenericDAO{
    private static List elements = null;
   	...
   	private static final String INSERT_SQL = "INSERT INTO disciplina ( den) " +
			"VALUES ( ?)";

	private static final String UPDATE_SQL = "UPDATE disciplina SET  den=? WHERE " +
			" id=? ";

	private static final String SELECT_SQL = "SELECT * FROM disciplina";

	private static final String SELECT_BY_PK_SQL = "SELECT * FROM disciplina WHERE " +
			" id=? ";
	private static final String SELECT_PURTARE_SQL = "SELECT * FROM disciplina WHERE " +
			" den='Purtare'";

    private static final String DELETE = "DELETE FROM disciplina WHERE " +
            " id=? ";
//    int[] a = new int[23];
   ...
   /* private static final String SELECT_LOGIN_SQL = "SELECT * FROM elev where username=? and userpassword=? ";  */
    ...
    private static final String COUNT_SQL = "SELECT count(*) FROM disciplina";


    private static final String DELETE_BY_PK_SQL = "DELETE FROM disciplina WHERE " +
            " id=? ";
    ...
    public long getIdPurtare(){
        elements = executeQueryParsed(SELECT_PURTARE_SQL);
        if(elements.size()>0)
            return ((DisciplinaBean)elements.get(0)).getId();
        return -1;
    }
