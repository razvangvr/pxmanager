 ...
 */
  public class DisciplineprofesoriDAO extends GenericDAO{
    private static List elements = null;

   	private static final String INSERT_SQL = "INSERT INTO disciplineprofesori ( id, id_clasa, id_disciplina, id_profesor, ore) " +
			"VALUES ( ?, ?, ?, ?, ?)";

	private static final String UPDATE_SQL = "UPDATE disciplineprofesori SET  id=?, id_clasa=?, id_disciplina=?, id_profesor=?, ore=? WHERE " +
			" id=? ";

	private static final String SELECT_SQL = "SELECT * FROM disciplineprofesori";

	private static final String SELECT_BY_PK_SQL = "SELECT * FROM disciplineprofesori WHERE " +
			" id=? ";

    private static final String DELETE = "DELETE FROM disciplineprofesori WHERE " +
            " id=? ";

    private static final String SELECT_BY_PROF_PK_SQL = "SELECT * FROM disciplineprofesori WHERE " +
            " id_profesor=? ";
    private static final String SELECT_BY_PROF_AND_AN_SQL = "SELECT * FROM disciplineprofesori, clasa WHERE " +
            " id_profesor=? and disciplineprofesori.id_clasa=clasa.id and id_an=?";
//    int[] a = new int[23];


   /* private static final String SELECT_LOGIN_SQL = "SELECT * FROM elev where username=? and userpassword=? ";  */

    private static final String COUNT_SQL = "SELECT count(*) FROM disciplineprofesori";


    private static final String DELETE_BY_PK_SQL = "DELETE FROM disciplineprofesori WHERE " +
            " id=? ";


    public List getByProfId(Object[] parameters){
        return executeQueryParsed(SELECT_BY_PROF_PK_SQL,parameters);
    }

     public List getByProfAn(String idprof, String idan){
        Object [] s = new Object[2];
         s[0] = idprof;
         s[1] = idan;
        return executeQueryParsed(SELECT_BY_PROF_AND_AN_SQL,s);
    }
