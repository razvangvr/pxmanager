 ...
 */
  public class AbsenteDAO extends GenericDAO{
    private static List elements = null;

   	private static final String INSERT_SQL = "INSERT INTO absente ( id, id_elev, motivat, id_an, nemotivat, semestru ) " +
			"VALUES ( ?, ?, ?, ?, ?, ? )";

	private static final String UPDATE_SQL = "UPDATE absente SET id_elev=?, motivat=?, id_an=?, nemotivat=?, semestru=? WHERE " +
			" id=? ";

	private static final String SELECT_SQL = "SELECT * FROM absente";

	private static final String SELECT_BY_PK_SQL = "SELECT * FROM absente WHERE " +
			" id=? ";

    private static final String DELETE = "DELETE FROM absente WHERE " +
            " id=? ";

    private static final String SELECT_BY_ELEV_PK_SQL = "SELECT * FROM absente WHERE " +
            " id_elev=? ";
    private static final String SELECT_BY_ELEV_AN_SQL = "SELECT * FROM absente WHERE " +
            " id_elev=? and id_an=?";
...
//    int[] a = new int[23];
   ...
   /* private static final String SELECT_LOGIN_SQL = "SELECT * FROM elev where username=? and userpassword=? ";  */
    ...
    private static final String COUNT_SQL = "SELECT count(*) FROM absente";


    private static final String DELETE_BY_PK_SQL = "DELETE FROM absente WHERE " +
            " id=? ";
