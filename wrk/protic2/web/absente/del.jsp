...
<%  int len = 0;
    ...
    List elements = absDAO.getByPK(new Object[] {request.getParameter("id")});
    mb = (AbsenteBean) elements.get(0);


%>

<h2 align=center>Stregere Absente</h2>
...
<%--        <jsp:include page="profilinc.jsp?idp=<%=ua.getId_profil()%>" flush="true"/>--%>
    ...
    <select name=sem>
           <option name="S1" type="text"  value="S1" <%=mb.getSemestru().equals("S1")?"Selected":""%>>S1</option>
           <option name="S2" type="text"  value="S2" <%=mb.getSemestru().equals("S2")?"Selected":""%>>S2</option>
    </select>        </td>
    </tr>
