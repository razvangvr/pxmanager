...
( function() {
	...
	var javascriptProtocolRegex = /^javascript:/,
		...
		contentLinkRegex = /^((?:news|articles|presentations|interviews|minibooks):\/\/)(.*)$/,
		resourceRegex = /^((?:resource):\/\/)(.*)$/,
	...
	CKEDITOR.plugins.link = {
		...
		parseLinkAttributes: function( editor, element ) {
			var href = ( element && ( element.data( 'cke-saved-href' ) || element.getAttribute( 'href' ) ) ) || '',
				...
				javascriptMatch, emailMatch, anchorMatch, urlMatch,ctUrlMatch,rUrlMatch,
				retval = {};
			...
			if ( !retval.type ) {
				...
				else if ( href && ( ctUrlMatch = href.match( contentLinkRegex ) ) ) {
					retval.type = 'contentLink';
					retval.ctUrl = {};
					retval.ctUrl.protocol = ctUrlMatch[ 1 ];
					retval.ctUrl.url = ctUrlMatch[ 2 ];
				}
				else if ( href && ( rUrlMatch = href.match( resourceRegex ) ) ) {
					retval.type = 'resource';
					retval.rUrl = {};
					retval.rUrl.protocol = rUrlMatch[ 1 ];
					retval.rUrl.url = rUrlMatch[ 2 ];
				}
				// url stays at the end because it matches anything!!!!!
		...
		getLinkAttributes: function( editor, data ) {
			...
			switch ( data.type ) {
				...
				case 'contentLink':
					var protocol = ( data.ctUrl && data.ctUrl.protocol != undefined ) ? data.ctUrl.protocol : 'news://',
							url = ( data.ctUrl && CKEDITOR.tools.trim( data.ctUrl.url ) ) || '';
						set[ 'data-cke-saved-href' ] = ( url.indexOf( '/' ) === 0 ) ? url : protocol + url;
						break;
						
				case 'resource':					
						url = ( data.rUrl && CKEDITOR.tools.trim( data.rUrl.url ) ) || '';
						set[ 'data-cke-saved-href' ] = ( url.indexOf( '/' ) === 0 ) ? url : "resource://" + url;
						break;
